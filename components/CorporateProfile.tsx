...
const CorporateProfile: React.FC<CorporateProfileProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 md:p-6 backdrop-blur-xl bg-[#051C2C]/80 animate-in fade-in duration-300 overflow-y-auto">
      <div className="bg-white w-full max-w-5xl min-h-screen md:min-h-0 md:h-[90vh] md:rounded-[2.5rem] shadow-2xl relative overflow-hidden flex flex-col">
        ...
        <div className="absolute top-6 right-6 flex gap-3 z-50 no-print">
           <button 
             onClick={handlePrint}
             className="bg-white/10 hover:bg-[#051C2C] hover:text-white text-[#051C2C] backdrop-blur-md border border-[#051C2C]/10 px-6 py-3 rounded-full font-black uppercase text-[10px] tracking-widest transition-all flex items-center gap-2"
           >
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar" id="corporate-profile-content">
           ...
           <div className="p-12 md:p-20 bg-white">
              ...
              <div className="grid md:grid-cols-3 gap-8">
                 {[
                   ...
                   <div key={i} className="bg-slate-50 p-8 rounded-[2rem] border border-slate-100 text-center">
